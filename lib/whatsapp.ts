...
/**
 * Envia o menu de assuntos como LISTA INTERATIVA do WhatsApp (o "seletor"
 * que o colaborador toca em vez de digitar). Usa o Content Template criado
 * uma única vez via `npm run setup:menu` (veja scripts/create-whatsapp-menu.ts),
 * cujo SID fica em TWILIO_MENU_CONTENT_SID. Se essa variável não estiver
 * configurada ainda, cai automaticamente para o menu em texto numerado —
 * o bot nunca fica travado esperando essa configuração.
 */
export async function sendSubjectMenu(params: {
  toPhone: string;
  prefixText?: string;
}): Promise<SendResult> {
  const client = getClient();
  ...
  if (!client || !from) {
    return {
      success: false,
      error: "Twilio não configurado (TWILIO_ACCOUNT_SID/TWILIO_AUTH_TOKEN/TWILIO_WHATSAPP_FROM).",
    };
  }
  ...
  const { buildSubjectMenuText } = await import("./subjects");
  const text = `${params.prefixText ? params.prefixText + "\n\n" : ""}${buildSubjectMenuText()}`;
  return sendFreeformWhatsApp({ toPhone: to, body: text });
}
